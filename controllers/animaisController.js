...
export async function criarAnimal(req, res) {
  const { nome, raca, especie, is_adotado, sobre, sexo, idade, peso, porte } =
    req.body;
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        mensagem: "Erro ao criar animal. Por segurança, tente novamente.",
      });
  }
}
...
export async function listarAnimais(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({
        mensagem: "Erro ao listar animais. Por segurança, tente novamente.",
      });
  }
}
...
export async function listarAnimaisPorResponsavel(req, res) {
  try {
    ...
    const animais = await AnimalServices.ListarAnimaisPorResponsavel(
      id,
      is_adotado
    );
    ...
    if (!animais) {
      return res
        .status(404)
        .json({ mensagem: "Responsável informado não existe." });
    }
    ...
    if (error.name === "CastError") {
      return res
        .status(400)
        .json({ mensagem: "Parâmetro 'Id do responsável' inválido." });
    }
...
export async function listarAnimaisPorLocalizacao(req, res) {
  try {
    ...
    const animais = await AnimalServices.ListarAnimaisPorLocalizacao(
      termo,
      is_adotado
    );
...
export async function listarAnimalPorId(req, res) {
  try {
    ...
    if (error.name === "CastError") {
      return res
        .status(400)
        .json({ mensagem: "Parâmetro 'Id do animal' inválido." });
    }
    console.error("Erro ao listar animal:", error);
    return res
      .status(500)
      .json({
        mensagem: "Erro ao listar animal. Por segurança, tente novamente.",
      });
  }
}
...
export async function alterarAnimal(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({
        mensagem: "Erro ao alterar animal. Por segurança, tente novamente.",
      });
  }
}
...
export async function deletarAnimal(req, res) {
  try {
    ...
    if (!result) {
      return res
        .status(404)
        .json({ mensagem: "Animal não encontrado para exclusão." });
    }
    ...
    return res
      .status(500)
      .json({
        mensagem: "Erro ao excluir animal. Por segurança, tente novamente.",
      });
  }
}
