...
export async function criarFormulario(req, res) {
  try {
    ...
    return res.status(500).json({
      mensagem: "Erro ao criar formulário. Por segurança, tente novamente.",
    });
  }
}
...
export async function listarFormularios(req, res) {
  try {
    ...
    return res.status(500).json({
      mensagem: "Erro ao listar formulários. Por segurança, tente novamente.",
    });
  }
}
...
export async function listarFormularioPorId(req, res) {
  try {
    ...
    if (error.name === "CastError") {
      return res
        .status(400)
        .json({ mensagem: "Parâmetro 'Id do formulário' inválido." });
    }
    console.error("Erro ao listar formulário:", error);
    return res.status(500).json({
      mensagem: "Erro ao listar formulário. Por segurança, tente novamente.",
    });
  }
}
...
export async function listarFormulariosPorLocalizacao(req, res) {
  try {
    ...
    if (cidade) {
      ...
      return res
        .status(400)
        .json({ mensagem: "Informe cidade ou estado para a busca." });
    }

    const formularios =
      await FormularioServices.ListarFormulariosPorLocalizacao(termo);

    if (!formularios || formularios.length === 0) {
      return res.status(404).json({
        mensagem: "Nenhum formulário encontrado para essa localização.",
      });
    }
...
// === READ BY LOCALIZAÇÃO ===
export async function listarFormulariosPorAnimal(req, res) {
  try {
    const animalId = req.params.id;

    const formularios = await FormularioServices.ListarFormulariosPorAnimal(
      animalId
    );

    if (!formularios || formularios.length === 0) {
      return res.status(404).json({
        mensagem: "Nenhum formulário encontrado para esse animal.",
      });
    }

    return res.status(200).json(formularios);
  } catch (error) {
    console.error("Erro ao listar formulários por animal:", error);
    return res.status(500).json({
      mensagem: "Erro ao listar formulários. Por segurança, tente novamente.",
    });
  }
...
export async function alterarFormulario(req, res) {
  try {
    ...
    const formAtualizado = await FormularioServices.AlterarFormulario(
      id,
      dados
    );
    ...
    if (!formAtualizado) {
      return res
        .status(404)
        .json({ mensagem: "Formulário não encontrado para alteração." });
    }
    ...
    return res.status(500).json({
      mensagem: "Erro ao alterar formulário. Por segurança, tente novamente.",
    });
  }
}
...
export async function deletarFormulario(req, res) {
  try {
    ...
    if (!result) {
      return res
        .status(404)
        .json({ mensagem: "Formulário não encontrado para exclusão." });
    }

    return res
      .status(200)
      .json({ mensagem: "Formulário excluído com sucesso." });
  } catch (error) {
    if (error.name === "CastError") {
      return res
        .status(400)
    ...
    return res.status(500).json({
      mensagem: "Erro ao excluir formulário. Por segurança, tente novamente.",
    });
  }
}
